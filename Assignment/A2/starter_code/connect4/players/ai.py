...
class AIPlayer:
    ...
    def expect_utility(self,state,depth,m,time_left):
        ...
        if(time_left<0.0001 or self.time-(time.time()-self.start_time)<0.1):
            if(self.player_number==1):
                return get_pts(self.player_number,board)-get_pts(2,board)+5*(num_popout[self.player_number].get_int()-num_popout[2].get_int())
            else:
                return get_pts(self.player_number,board)-get_pts(1,board)+5*(num_popout[self.player_number].get_int()-num_popout[1].get_int())
        if(m==0):
    ...
    def get_expectimax_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        actions=get_valid_actions(self.player_number,state)
        weights=[None]*len(board[0])
        c=len(weights)/2
        for i in range(len(weights)):
            weights[i]=1#(1-abs(i+1-c)/(c*10))
        depth=15
        utility=0
        index=0        
        for i in range(len(actions)):
            ...
            if(b and num_popout[self.player_number].get_int()>0):
                num_popout[self.player_number].decrement()
                state1=board1,num_popout
                u=weights[col]*self.expect_utility(state1,depth,0,time_left)
                if(u>utility):
                    utility=u
                    index=i
                num_popout[self.player_number].increment()
    ...
    def max_value(self,state, alpha, beta,depth,time_left):
        ...
        for a in get_valid_actions(self.player_number,state):
            ...
            if(b and num_popout[self.player_number].get_int()>0):
                num_popout[self.player_number].decrement()
                state1=board1,num_popout
                utility= max(utility, self.min_value(state1, alpha, beta,depth-1,t))
                num_popout[self.player_number].increment()
            else:
                state1=board1,num_popout
                utility= max(utility, self.min_value(state1, alpha, beta,depth-1,t))
    ...
    def min_value(self,state, alpha, beta,depth,time_left):
        ...
        for a in get_valid_actions(other_player,state):
            ...
            if(b and num_popout[other_player].get_int()>0):
                num_popout[other_player].decrement()
                state1=board1,num_popout
                utility= min(utility, self.max_value(state1, alpha, beta,depth-1,t))
                num_popout[other_player].increment()
            else:
                state1=board1,num_popout
                utility= min(utility, self.max_value(state1, alpha, beta,depth-1,t))
    ...
    def get_intelligent_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        for i in range(len(actions)):            
            ...
            if(b and num_popout[self.player_number].get_int()>0):             
                num_popout[self.player_number].decrement()
                state1=board1,num_popout
                u=weights[col]*self.min_value(state1,utility,np.inf,depth,time_left)
                if(u>utility):
                    utility=u
                    index=i
                num_popout[self.player_number].increment()
